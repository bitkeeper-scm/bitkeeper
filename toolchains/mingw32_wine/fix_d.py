...
# Replace Z: drive letters and remove backslash continuation escapes
content = content.replace("Z:", "").replace("\\\n", " ").replace("\\\r\n", " ")
# Normalize backslashes inside path names to forward slashes
...
for tok in tokens:
    if tok.endswith(":"):
        ...
        tok = tok.replace("\\", "/")
        if tok.startswith(tool_dir):
            # Ignore toolchain builtin system headers
            continue
        else:
            new_tokens.append(tok)
