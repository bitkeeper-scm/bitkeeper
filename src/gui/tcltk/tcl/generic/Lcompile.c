...
compile_fnDecl(FnDecl *fun, Decl_f flags)
{
	...
	if ((L->options & L_OPT_FNTRACE) &&
		...
		ASSERT(L->fnhook);
		push_lit("L_fn_pre_hook");
		push_lit(L->fnhook);
		push_litf("%d", num_parms+1);
		push_lit("::list");
		push_lit(name);
 ...
 * interpolated, we can't tell for sure, so assume the worst.  For the
 * first two cases, if ds is non-NULL return the equivalent glob in
 * *ds; this becomes an operand to INST_STR_EQ or INST_STR_MATCH.
 */
private ReKind
