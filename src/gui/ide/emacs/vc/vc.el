...
;; XEmacs conversion: Steve Baur <steve@xemacs.org>
...
(defmacro vc-backend-dispatch (f b s r c)
  "Execute FORM1, FORM2, FORM3 or FORM4 for BitKeeper, SCCS, RCS or CVS 
respectively. If FORM4 is `RCS', use FORM3 for CVS as well as RCS.
\(CVS shares some code with RCS)."
  (list 'let (list (list 'type (list 'vc-backend f)))
	(list 'cond
	      (list (list 'eq 'type (quote 'BK)) b) 	;; BitKeeper
...
(defcustom vc-default-init-version nil
...
(defcustom vc-file-not-found-use-default nil
  "*If non-nil, just check out the default version.
If `nil', then this will use the working file when checking out an
unfound file."
  :type 'boolean
  :group 'vc)
...
;;; XEmacs:
;;; This was originally diff-switches, but that name belongs to
;;; another package that we don't want to require just to be able
;;; to use VC.
(defvar vc-diff-switches (if (boundp 'diff-switches) diff-switches "-c")
...
(defcustom vc-bk-release nil
  "*The release number of your BitKeeper installation, as a string.
If nil, VC itself computes this value when it is first needed."
  :type '(choice (const :tag "Auto" nil)
		 string)
  :group 'vc)
  ...
  (cond
   ...
   ((eq backend 'CVS)
	...
	(setq vc-cvs-release 'unknown)))
     ...
     ((eq backend 'BK)
      (or vc-bk-release
	  (and (zerop (vc-do-command nil 1 "bk" nil nil "version"))
	     (save-excursion
	       (set-buffer (get-buffer "*vc*"))
	       (setq vc-bk-release
		     (car (vc-parse-buffer
			   '(("^BitKeeper version is \\([0-9.]+\\)"
			      1)))))))
	(setq vc-cvs-release 'unknown)))
     ((eq backend 'SCCS)
      vc-sccs-release)))

;;; XXX (vc-backend-release 'BK)
...
(defun vc-latest-on-branch-p (file)
  ...
  (vc-backend-dispatch file
     ;; BitKeeper
     t
     ;;; XXX (string= (vc-workfile-version file) (vc-latest-version file))
     ;; SCCS
...
to an optional list of FLAGS."
  ...
  (let ((obuf (current-buffer)) (camefrom (current-buffer))
	...
	(progn
	  ...
	  (error "Running (%s %s)...FAILED (%s)" command squeezed
		 (if (integerp status)
...
(defun vc-buffer-context ()
  ...
  (let ((point-context (vc-position-context (point)))
	...
	(mark-context  (if (eq (marker-buffer (mark-marker #+xemacs t))
			       (current-buffer))
			   (vc-position-context (mark-marker #+xemacs t))))
...
(defun vc-clear-headers ()
  ...
  (let ((context (vc-buffer-context)))
    (goto-char (point-min))
    ; FSF uses explicit keywords here, is that safer? JV
...
(defun vc-rename-file (old new)
  ...
  (if (eq (vc-backend old) 'BK)
      (error "Add renaming code 'bk mv old new'"))
...
; to some checked in version.  However, testing for this is tricky....
  ...
  (vc-backend-dispatch old
		       (vc-record-rename old new) ;BitKeeper
...
default directory."
  ...
  (let ((odefault default-directory)
    ...
    (message "Computing change log entries... %s"
	     ; The FSF version calls rcs2log with a "-c"
	     ; argument that we don't seem to have
	     ; It also catches errors more properly JV
...
;;; XXX Add capability for BitKeeper XXX
;;;###autoload
...
;; time-span of assoc-list.  Optionally quantize to the factor of
;; quantize.
...
(defun vc-backend-admin (file &optional rev comment)
  ...
  (or vc-default-back-end
	  ...
	  (cond
	   ((vc-find-binary "bk") 'BK)
	   ((vc-find-binary "rcs") 'RCS)
	   (t 'SCCS))))
  ...
  (let ((switches
    ...
    (cond ((eq backend 'BK)
	   (apply 'vc-do-command nil 0 "bk" file 'WORK ;; BitKeeper
                                 "new"
                                 (and comment (concat "-y" comment))
                                 switches)
	   (if vc-keep-workfiles
	       (vc-do-command nil 0 "bk" file 'WORK "get")))
	  ((eq backend 'SCCS)
	...
	(save-excursion
	  ...
	  (vc-backend-dispatch file
		...
		(apply 'vc-do-command
		       ...
		       (if writable "-l")
		       (if rev (concat "-r" rev)
			 (if (not vc-file-not-found-use-default)
...
(defun vc-backend-checkin (file rev comment)
  ...
  (save-excursion
	...
	;; BitKeeper
	(progn
	  (apply 'vc-do-command nil 0 "bk" file 'WORK
		 "delta"
		 (if rev (concat "-r" rev))
		 (concat "-y" comment)
		 switches)
	  ...
	  (vc-file-setprop file 'vc-workfile-version nil)
	  (if vc-keep-workfiles
	      (vc-do-command nil 0 "bk" file 'WORK "delta"))
	  )
	;; SCCS
	(progn
...
(defun vc-backend-revert (file)
  ...
  (vc-backend-dispatch
   file
   ;; BitKeeper XXX: check on the 'nil in vc-do-command
   (progn
     (vc-do-command nil 0 "bk" file 'WORK "unedit")
     (vc-do-command nil 0 "bk" file 'WORK "get"))
   ;; SCCS
   (progn
...
(defun vc-backend-steal (file &optional rev)
  ...
  (vc-backend-dispatch file
   nil ;; XXX: Does this make sense for BitKeeper
   (progn				;SCCS
...
(defun vc-backend-uncheck (file target)
  ...
  (vc-backend-dispatch file
   nil  ;; can't do this in BitKeeper
...
(defun vc-backend-print-log (file)
  ...
  (vc-backend-dispatch 
   file
   (vc-do-command nil 0 "prs" file 'WORK)
...
(defun vc-backend-assign-name (file name)
  ...
  (vc-backend-dispatch file
   (vc-add-triple name file (vc-latest-version file))              ;; BitKeeper
...
(defun vc-backend-diff (file &optional oldvers newvers cmp)
  ...
  (let ((backend (vc-backend file)) options status
        (diff-switches-list (if (listp vc-diff-switches) 
                                vc-diff-switches 
	  ...
	  (if (or oldvers newvers)
	    ...
	    (if cmp 1 ;; file is added but not committed, 
	      ...
	      (apply 'vc-do-command
		     ...
		     (append (if (listp vc-diff-switches) 
				 vc-diff-switches
...
(defun vc-backend-merge-news (file)
  ...
  (prog1
      (vc-backend-dispatch 
       file
       ;; XXX: Need to check to see whether we should put 'bk pull' here
       (error "vc-backend-merge-news not meaningful for BitKeeper files") ;BK
...
(defun vc-check-headers ()
  ...
  (save-excursion
    ...
    (vc-backend-dispatch buffer-file-name
     (re-search-forward  "%[MIRLBSDHTEGUYFPQCZWA]%" nil t)      ;; Bitkeeper XXX:Update
