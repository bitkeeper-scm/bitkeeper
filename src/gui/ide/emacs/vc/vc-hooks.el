...
(defcustom vc-default-back-end nil
  "*Back-end actually used by this interface; may be SCCS or RCS, BK.
The value is only computed when needed to avoid an expensive search."
  :type '(choice (const nil) (const RCS) (const SCCS) (const BK))
  :group 'vc)
...
(defun vc-parse-locks (file locks)
  ...
  (if (not locks) 
    ...
    (let ((found t) (index 0) master-locks version user)
      (cond ((or (eq (vc-backend file) 'SCCS) (eq (vc-backend file) 'BK))
...
(defun vc-fetch-master-properties (file)
  ...
  (save-excursion
    (cond
     ((or (eq (vc-backend file) 'SCCS) (eq (vc-backend file) 'BK))
...
(defun vc-get-dir (&optional path)
  "Helper function for vc-bkroot-p"
  (if path 
      (cond ((file-directory-p path) (expand-file-name path))
	    (t (file-name-directory path)))
    (expand-file-name default-directory)))

(defun vc-bkroot-p (&optional path)
  "Return true if we are in a bitkeeper repository"
  (interactive "sDirectory:")
  (let ((dir (vc-get-dir path)))
    (if (eq 0 (apply 'call-process "bk" nil nil nil (list "root" dir)))
	1
      0)))

;;; (setq default-directory "/tmp")
;;; (setq default-directory "/tmp/UserGuide")
;;; (message (format "rt=(%s)" (vc-bkroot-p)))
;;; (message (format "rt=(%s)" (vc-bkroot-p "/tmp/UserGuide")))
;;; (message (format "rt=(%s)" (vc-bkroot-p "/tmp/UserGuide/new.c")))
;;; (message (format "rt=(%s)" (vc-bkroot-p "/tmp/junk")))
;;; (message (format "rt=(%s)" (vc-get-dir "/tmp/UserGuide/new.c")))
  ...
  (progn
    ...
    (symbol-name
     (or
      ...
      (cond
       ((eq vc-default-back-end 'BK) 'SCCS)
       ((eq vc-default-back-end 'RCS) 'RCS)
       (t 'SCCS))))))
...
(defun vc-workfile-version (file)
  ...
  (cond ((vc-file-getprop file 'vc-workfile-version))
	...
	((eq (vc-backend file) 'BK) (vc-latest-version file))
	((eq (vc-backend file) 'RCS)
...
;;; XXX: Need to put in test to make sure that the BK type is really
;;; a BitKeeper SCCS file and not an ATT SCCS file
...
;;; I chose to make a search-bk-project dir since we will probably
;;; move the SCCS directory into the BitKeeper/ hierarchy at some
;;; point. When we do that, then we will need to create a
;;; vc-bk-project-dir defun
(defun vc-search-bk-project-dir (dirname basename)
  ;; Check if there is a master file for BASENAME in the 
  ;; BK metadata directory.  If yes, throw `found' as
  ;; expected by vc-registered.  If not, return nil.
  (let* ((project-dir (vc-sccs-project-dir))
         (master-file (and project-dir (concat project-dir "s." basename))))
    (and master-file
         (file-exists-p master-file)
         (throw 'found (cons master-file 'BK)))))
