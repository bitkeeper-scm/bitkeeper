...
rcs_parselog_rdelta(MMAP *m)
{
	...
	expect(m, "revision ");
	delta->rev = dup_eol(m);
	if (t = index(delta->rev, '\t'))
		*t = '\0'; /* skip 'locked by: guido' */

	expect(m, "date: ");
	delta->sdate = dup_tochar(m, ';');
	expect(m, "  author: ");
	delta->author = dup_tochar(m, ';');
	t = delta->author;	/* space are not allowed */
	while (*t) {
		if (isspace(*t)) *t = '_';
		t++;
	}
	...
	free(state);
	skip_eol(m);
	if (strneq(m->where, "branches: ", 10)) skip_eol(m);
				/* skip branches: line */
	t = mwhere(m);
	while (!end_of_comment(m->where)) skip_eol(m);
	delta->comments = malloc(m->where - t + 1);
	strncpy(delta->comments, t, m->where - t);
	delta->comments[m->where - t] = 0;
	return delta;
}


/*
 * Return true if the first difference between a and b
 * occurs after the last instance of c is either string.
 */
private int
same_prefix(const char *a, const char *b, int c)
{
	while (*a && *b &&*a == *b)
		a++, b++;
	if (index(a, c) || index(b, c))
		return 0;
	return 1;
}
...
rcs_parselog(MMAP *m)
{
	const char *p;  /* pointer into mmap */
	const char *start;
	char	*kk;
	RCS	*rcs;
	int	i;
	...
	expect(m, "RCS file: ");
	rcs->rcsfile = dup_eol(m);
	expect(m, "Working file: ");
	rcs->workfile = dup_eol(m);
	expect(m, "head: ");
	skip_eol(m);
	expect(m, "branch:");
	if (*m->where == ' ') {
		expect(m, " ");
		rcs->defbranch = dup_eol(m);
	} else {
		skip_eol(m);
	}
	expect(m, "locks:");
	skip_eol(m);		/* strict or not, I don't care */
	while (*m->where == '\t') {   /* Ignore locks */
		skip_eol(m);
	}
	expect(m, "access list:\n");
	expect(m, "symbolic names:\n");
	while (*m->where == '\t') {
		skip_eol(m);
	}
	expect(m, "keyword substitution: ");
	kk = dup_eol(m);
	rcs->kk = aprintf("-k%s", kk);
	free(kk);
	expect(m, "total revisions: ");
	skip_eol(m);
	...
	if (m->where > start) {
		rcs->text = malloc(m->where - start + 1);
		strncpy(rcs->text, start, m->where - start);
		rcs->text[m->where - start] = 0;
	} else {
		rcs->text = 0;
	}
	while (*m->where == '-') {
		rdelta *d = rcs_parselog_rdelta(m);
		if (rcs->table) {
			int c = strcnt(d->rev, '.');
			int cprev = strcnt(rcs->table->rev, '.');

			if ((c == cprev) &&
			    (c == 1 ||
				same_prefix(rcs->table->rev, d->rev, '.'))) {
				rcs->table->snext = strdup(d->rev);
			}
		}
		rcs->n++;
		if (d->next = rcs->table) rcs->table->prev = d;
...
/*
 * Parse a CVS log command output and create a structure
 * containing all the meta data from a CVS repository.
 * Automaticly find deltas and csets.
 */
CVS *
cvs_init(char *file)
{
