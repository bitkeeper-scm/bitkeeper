...
STDMETHODIMP
CBkIgnoredIcon::GetOverlayInfo(LPWSTR pwszIconFile, int cchMax,
    int *pIndex, DWORD *pdwFlags)
{
	cchMax -= MultiByteToWideChar(CP_ACP, NULL, bkdir, -1,
	    pwszIconFile, cchMax) + 1;
	wcsncat_s(pwszIconFile, cchMax, L"\\icons\\BkIgnored.ico", cchMax);
	*pIndex = 0;
...
STDMETHODIMP
CBkIgnoredIcon::GetPriority(int *pPriority)
{
	*pPriority = 0;
	return (S_OK);
}
...
STDMETHODIMP
CBkIgnoredIcon::IsMemberOf(LPCWSTR pwszPath, DWORD dwAttrib)
{
	int	state;
	struct	stat sb;
	char	buf[MAXPATH];
	...
	if (state == BK_IGNORED) return (S_OK);
	return (S_FALSE);
}
