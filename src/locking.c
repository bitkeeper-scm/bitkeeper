...
rdlock(void)
{
	...
	write_log(root, "cmd_log", 0, "obtain read lock (%u)", getpid());
	ldebug(("RDLOCK %u\n", getpid()));
	return (0);
}
...
wrlock(void)
{
	...
	write_log(root, "cmd_log", 0, "obtain write lock (%u)", getpid());
...
repository_rdunlock(int all)
{
	...
	rdlockfile(root, path);
	if (unlink(path) == 0) {
		write_log(root, "cmd_log", 0, "read unlock (%u)", getpid());
		ldebug(("RDUNLOCK %u\n", getpid()));
	}
	sprintf(path, "%s/%s", root, READER_LOCK_DIR);
	rmdir(path);
