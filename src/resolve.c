...
/*
 * Make sure the file name did not change case orientaion after we copied it
 */
private int
chkCaseChg(char *list)
{
	FILE	*f;
...
err:			unapply(save);
	...
	fflush(save); /*  important */
 	...
 	/* if cse folding FS, make sure file name did not change case */
	if (isCaseFoldingFS() && chkCaseChg(APPLIED)) goto err;
