...
LM TODO's
   ...
   - I really dislike the bp_insert() step in clone.  It delays long clones
     needlessly.
     (WAYNE: what do you want to do instead? Just transfering the whole binpool
   ...
   - interrupting a bp_transfer does not kill the sfio on the remote side
     (WAYNE: I think it will get EOF and clean up after itself now.  Unless
	     it happens to be broken on a file boundry.)
   ...
   - I finally remember why I didn't like the binpool_send/receive.  They
     ...
     that gets you the binpool behaviour.
----------------------------------------------------------
Binpool servers
	...
	transfered to the binpools servers at both ends of the
	transfer.
...
bkd_clone:
	- push binpool deltas to be transfered to the binpool server
	- send SFIO
