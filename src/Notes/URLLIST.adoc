...
information, we keep the information in two separate files.
  ...
  beacuse older BKs (bk-5.1) used to store it. It is deprecated.
...
Besided this new structure `urlinfo`, a couple of fields have been
...
=== API ===
    ...
    intializes the `n->urls` array.

    `urlinfo_urlArgs()`:: Occassionally we need to check some extra
