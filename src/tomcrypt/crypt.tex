...
\makeindex
\begin{document}
\title{Tiny Crypto Library \\ ``LibTomCrypt'' \\ Version 0.62}
\author{Tom St Denis \\
Algonquin College \\
\\
...
length as the block size of the cipher you choose.  It is important that the IV  be random for each unique message you 
want to encrypt.  The parameters ``key'', ``keylen'' and ``num\_rounds'' are the same as in the XXX\_setup() function call.  
The final parameter is a pointer to the structure you want to hold the information for the mode of operation.
...
int main(void)
{
   ...
   /* call the hash */
   hash_memory(idx, "hello world", 11, out);
}
\end{verbatim}
\end{small}
...
with 1280 and above bit RSA keys.  You can use SHA-512 with 1792 and above bit RSA keys.
...
example program that communicates over sockets, using MD5 and 1024-bit DH keys is:
\newpage
\begin{small}
\begin{verbatim}
int establish_secure_socket(int sock, unsigned char *key, 
...
The ``keysize'' is the size of the modulus in bytes desired.  Currently directly supported values are 24, 28, 32, 48 and 65 which
