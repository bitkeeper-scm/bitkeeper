...
\makeindex
\begin{document}
\title{A Tiny Crypto Library, \\ LibTomCrypt \\ Version 0.70}
\author{Tom St Denis \\
Algonquin College \\
\\
...
to provide these low level core functions.  
...
for you.  The library has been written from scratch to provide basic functions as well as non-standard higher level 
functions.  
...
All of the code except for MPI and a few of the ciphers and hash functions have been written 
...
int main(void)
{
   ...
   if (ctr_encrypt(buffer, buffer, sizeof(buffer), &ctr) == CRYPT_ERROR) {
...
int main(void)
{
   ...
   if (rc4_init(&prng) == CRYPT_ERROR) {
      printf("RC4 init error: %s\n", crypt_error);
      exit(-1);
   }

   /* use ``key'' as the key */
   if (rc4_add_entropy("key", 3, &prng) == CRYPT_ERROR) {
      printf("RC4 add entropy error: %s\n", crypt_error);
      exit(-1);
   }

   /* setup RC4 for use */
   if (rc4_ready(&prng) == CRYPT_ERROR) {
