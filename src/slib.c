 ...
 * sc has had the graph modified with some nodes removed (D_GONE),
 * and parent, kid, and merge reworked.
 * scb is the original.
 */

void
sccs_adjustSet(sccs *sc, sccs *scb, delta *d, u8 *slist)
{
	delta	*old_left, *old_right;
	delta	*new_left, *new_right;
	int	count;

	assert(d && slist);	/* and slist is all 0 */

	new_left = d;		/* sc is new graph; d is in sc */
	new_right = d->parent;

	old_left = sfind(scb, d->serial);
	old_right = sfind(scb, d->pserial);
	assert(old_left && old_right);

	/*
	 * symdiff with count < 0 returns slist = slist ^ symdiff(l, r)
	 * initial slist is empty, so it is just returning the symdiff.
	 * The return value is how many elements were altered.
	 * Since initial slist is zero, it returns how many nonzero entries
	 */
	count = graph_symdiff(old_left, old_right, slist, -1);

	/*
	 * symdiff with count will do a compress into the include and
	 * exclude list of new_left.  It will ignore anything in those
	 * lists and set them such that symdiff of new_left and new_right
	 * will be the initial slist with D_GONE removed.
	 * slist will be empty on return and return value 0
	 */
	count = graph_symdiff(new_left, new_right, slist, count);
	assert(!count);
}
...
delta_table(sccs *s, FILE *out, int willfix)
{
	...
	for (d = s->table; d; d = NEXT(d)) {
		 ...
		 */
		if (d->pserial && BITKEEPER(s) &&
		    (d->date <= PARENT(s, d)->date)) {
		    	s->state |= S_READ_ONLY;
			fprintf(stderr,
		...
		*p = '\0';
		...
		if (d->flags & D_MODE) {
		    	unless (d->pserial && sameMode(PARENT(s, d), d)) {
				p = fmts(buf, "\001cO");
	    ...
	    (d->r[2] && (!d->r[3] && !d->random)))
	{
		unless (flags & ADMIN_SHUTUP) {
			fprintf(stderr, "%s: bad revision %s (parent = %s)\n",
			    file, d->rev, d->pserial?PARENT(s, d)->rev:"Root");
		}
		error = 1;
	}
	/*
	...
	if (d->date < p->date) {
		if (flags & ADMIN_TIME) {
			fprintf(stderr,
			    ...
			    file, d->rev, PARENT(s, d)->rev);
			fprintf(stderr, "\t%s: %s    %s: %s -> %d seconds\n",
			    d->rev, d->sdate, PARENT(s, d)->rev, PARENT(s, d)->sdate,
...
do_patch(sccs *s, delta *d, int flags, FILE *out)
{
	...
	if (!d) return (0);
	type = d->type;
	if ((d->type == 'R') && PARENT(s, d) && streq(d->rev, PARENT(s, d)->rev)) {
	    	type = 'M';
	}
